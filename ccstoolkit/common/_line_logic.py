...
def _form_lines(_rules,_lines):
	lines = dict()
	for key,rules in _rules.items():
		rules_f = list()
		for rule in rules:
			if rule[0]=='above':
				rules_f.append(lambda x, y, P, key=rule[1]: y > _lines[key]['y'](x,P))
			elif rule[0]=='below':
				rules_f.append(lambda x, y, P, key=rule[1]: y < _lines[key]['y'](x,P))
			elif rule[0]=='right of':
				rules_f.append(lambda x, y, P, key=rule[1]: x > _lines[key]['x'](y,P))
			elif rule[0]=='left of':
				rules_f.append(lambda x, y, P, key=rule[1]: x < _lines[key]['x'](y,P))
				
		lines[key] = _lines[key] | {'rules': rules, 'rules_f': rules_f}
...
def _get_regions(lines: dict, P: dict, x_bounds: tuple, y_bounds: tuple):
	...
	#Separate the edges of the bounding box	#It's important that they are both sorted and unique!
	left_right = [sorted(set([(x,y) for x,y in intersections_box+corners if x==xlim]),key=lambda p: p[1]) for xlim in x_bounds]
	top_bottom = [sorted(set([(x,y) for x,y in intersections_box+corners if y==ylim]),key=lambda p: p[0]) for ylim in y_bounds]
	...
	for edge in edges:
		...
		#face = list()
		face = {'bounds ids': [], 'points': []}
		current = edge
		
		#For the traversal
		visited = set()
		
		#Add all 'from' points until the loop closes
		while True:
			current_id = current['id']						 #Where we are at

			if current_id in visited:						  #If already visited
			
				if current_id == edge['id']:				   #If we just now closed the edge, no issue
					break
				else:										   #Else big issue
					raise RuntimeError(
						f"Invalid face traversal: cycle detected. "
						f"Started at {edge['id']}, "
						f"re-entered {current_id}. "
						f"Path: {face['bounds ids']}"
					)
			
			visited.add(current_id)							#Save the id in visited
	
			current['used'] = True							 #Remove this edge from rotation
			
			face['bounds ids'].append(current['id'])
			...
			for idx, oe in enumerate(outs):
				if oe['to'] == current['from']:
					break
			#If for never breaks, i.e. no reverse edge?
			else:
				raise RuntimeError(f"Missing reverse edge for {current_id}")
	...
	regions = [{'bounds ids': region['bounds ids'], 'points': region['points']+[region['points'][0]]} for region in regions]
	...
	for region in inner_regions:
		region['centroid'] = list(_math._calculate_centroid(region['points']))
		
	#Sort the regions and keys
	key_order = ['bounds ids','area','centroid','points']
	inner_regions = sorted([{key: region[key] for key in key_order if key in region} for region in inner_regions], key=lambda d: d["bounds ids"])
